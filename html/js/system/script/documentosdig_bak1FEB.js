Dynamsoft.WebTwainEnv.RegisterEvent('OnWebTwainReady', Dynamsoft_OnReady2); // Register OnWebTwainReady event. This event fires as soon as Dynamic Web TWAIN is initialized and ready to be used
...
function activarControles(activar,controles){
		...
		if (activar){
			cursor="default"; 
			cursor1="pointer";
			estilo="btn btn-default disabled";
			estilo1="btn btn-default"
		}
		else{
			cursor="pointer"; 
			cursor1="default";
			estilo="btn btn-default";
			estilo1="btn btn-default disabled"
		}
...
function AcquireImage(myDWObject,controles,doble) {
    if (myDWObject) {
        myDWObject.SelectSourceByIndex(0); 
        myDWObject.IfFeederEnabled = true;
        myDWObject.IfAutoScan = true;        
        var bSelected = myDWObject.EnableSource();
        if (bSelected) {
      ...
      //      if(doble=='true'&&document.getElementById("chckINE").checked)
///            	myDWObject.IfFeederEnabled = true;
